data_path <- "data_HL.csv"
out_csv <- "HL_anova.csv"
out_txt <- "HL_anova.txt"
...
log_analysis <- anova_result(dat$log_density, dat$month, "Log-transformed total tick density")

welch_raw <- oneway.test(density ~ month, data = dat, var.equal = FALSE)
welch <- data.frame(
...
txt <- c(
  "HL monthly total tick density ANOVA analysis",
  "============================================",
  "",
  ...
  "Workflow",
  "--------",
  "Each original sampling record was treated as one observation. Monthly differences in total tick density were first assessed using one-way ANOVA. Normality was checked using the Shapiro-Wilk test applied to ANOVA residuals, and homogeneity of variances was checked using a median-centered Levene test. Tukey's HSD test was used for post-hoc multiple comparisons. Because raw tick density was right-skewed and the Shapiro-Wilk test rejected normality, a log-transformed ANOVA was additionally performed as a sensitivity analysis. Welch's ANOVA was also calculated for transparency.",
  "",
  "Descriptive result",
  "------------------",
  ...
  paste0("Log-density ANOVA residual normality: Shapiro-Wilk W = ",
         sprintf("%.4f", log_analysis$shapiro$W), ", P = ", fmt_p(log_analysis$shapiro$p_value), "."),
  paste0("Log-density homogeneity of variances: Levene F(",
  ...
  paste0("Log-density one-way ANOVA: F(",
  ...
  if (log_sig) {
    paste0("The log-transformed sensitivity analysis also indicated significant monthly differences (P = ",
           fmt_p(log_analysis$overall$p_value), ").")
  } else {
    paste0("The log-transformed sensitivity analysis did not indicate significant monthly differences (P = ",
           fmt_p(log_analysis$overall$p_value), ").")
  },
  paste0("Number of significant Tukey HSD pairwise comparisons on raw density: ", nrow(raw_tukey_sig), "."),
  paste0("Number of significant Tukey HSD pairwise comparisons on log density: ", nrow(log_tukey_sig), "."),
  "",
  "Suggested Methods text",
  "----------------------",
  "Differences in monthly total tick density were assessed using one-way analysis of variance (ANOVA), with each sampling record treated as an independent observation. Prior to ANOVA, normality was evaluated using the Shapiro-Wilk test applied to model residuals, and homogeneity of variances was assessed using a median-centered Levene test. Tukey's honestly significant difference (HSD) test was used for post-hoc multiple comparisons. Because the raw-density residuals deviated from normality, a log-transformed ANOVA was additionally conducted as a sensitivity analysis, and Welch's ANOVA was calculated to assess robustness to unequal variances.",
  "",
  "Suggested Results text",
  "----------------------",
  paste0("For raw total tick density, the Shapiro-Wilk test indicated deviation from normality (W = ",
         sprintf("%.4f", raw_analysis$shapiro$W), ", P = ", fmt_p(raw_analysis$shapiro$p_value),
         "), whereas Levene's test did not indicate heterogeneity of variances [F(",
         raw_analysis$levene$df1, ", ", raw_analysis$levene$df2, ") = ",
         sprintf("%.4f", raw_analysis$levene$F), ", P = ", fmt_p(raw_analysis$levene$p_value), "]. ",
         "One-way ANOVA did not detect a significant difference in monthly total tick density [F(",
         raw_analysis$overall$df1, ", ", raw_analysis$overall$df2, ") = ",
         sprintf("%.4f", raw_analysis$overall$F), ", P = ", fmt_p(raw_analysis$overall$p_value),
